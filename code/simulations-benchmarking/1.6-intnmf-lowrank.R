...
for (fold_ in folds) {
  ...
  for (nfeats in fsignals) {
    ...
    #shift values to meet nonnegativity constraint
    if (!all(f_100.data>=0)) f_100.data <- pmax(f_100.data + abs(min(f_100.data)), .Machine$double.eps)
    f_100.data <- f_100.data/max(f_100.data)
    
    if (!all(f_150.data>=0)) f_150.data <- pmax(f_150.data + abs(min(f_150.data)), .Machine$double.eps)
    f_150.data <- f_150.data/max(f_150.data)
    
    if (!all(f_200.data>=0)) f_200.data <- pmax(f_200.data + abs(min(f_200.data)), .Machine$double.eps)
    f_200.data <- f_200.data/max(f_200.data)
    
    factorizations_intnmf <- nmf.mnnals(dat=list(f_100.data, f_150.data, f_200.data), k=3)
    loadings.out <- as.data.frame.matrix(factorizations_intnmf$W)
    rownames(loadings.out) <- ordered_names_save
    write.csv(loadings.out, paste("../../data/simulations/lowrank/intNMF_fsignal/",
