...
def mahalanobis_centroid_distance(res_df, pcs=['PC1','PC2','PC3'],
                                  meta_col='diagnosis', regularization=1e-6):
    """
    ...
    res_df = res_df.dropna(subset=[meta_col]).copy()
    X = res_df[pcs]
    y = res_df[meta_col].values
    
    classes = np.unique(y)
    assert len(classes) == 2, "Binary labels required."
    ...
    pooled_cov += np.eye(pooled_cov.shape[0]) * regularization
